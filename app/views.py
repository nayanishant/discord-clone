...
def userProfile(request, pk):
    ...
    context = {'user': user, 'rooms': rooms, 'room_messages': room_messages, 'topics': topics, 'about': about}
...
def update_room(request, pk):
    ...
    if request.user != room.host:
        return HttpResponse('Action restricted')
...
def create_user(request):
    ...
    if request.method == 'POST':
        ...
        if len(password) < 8 or password.isalnum() is False or username == password:
        ...
        if password == password2:
            ...
            if len(password) < 8 and password.isalnum() is False:
                messages.error(request, "Try another password")
        else:
...
def updateUser(request):
    ...
    return render(request, 'app/user_update.html', {'form': form, 'about': about})
